...
/******* lignes (en rouge, car parcours selon X) *********/
	...
	caneva_pts.x[5] = cross_down.x+lx*0.37;	//encadre la 2eme colone d'image
	caneva_pts.x[6] = cross_down.x+lx*0.48;

	caneva_pts.x[7] = cross_down.x+lx*0.55;	//encadre la 3eme colone d'image
	caneva_pts.x[8] = cross_down.x+lx*0.66;

	caneva_pts.x[9] = cross_down.x+lx*0.72;	//encadre la 4eme colone d'image
	caneva_pts.x[10] = cross_down.x+lx*0.84	;

	caneva_pts.x[11] = cross_down.x+lx*0.9;	//encadre la 5eme colone d'image
	caneva_pts.x[12] = cross_down.x+lx*1.02;

	caneva_pts.x[LAST_VERTICAL_COL] =  cross_down.x+lx*1.05 ;//==la fin de l'image, dernière bordure d'arret
...
/******* colones (en bleu, car parcours selon Y)*********/
	...
	caneva_pts.y[1] = cross_up.y + ly*0.1;		//encadre la 1eme ligne d'imagette dessinée main
	caneva_pts.y[2] = cross_up.y + ly*0.18 ;

	caneva_pts.y[3] = cross_up.y + ly*0.22 ;	//encadre la 2eme ligne d'imagette dessinée main
	caneva_pts.y[4] = cross_up.y + ly*0.31 ;

	caneva_pts.y[5] = cross_up.y + ly*0.35 ;	//encadre la 3eme ligne d'imagette dessinée main
	caneva_pts.y[6] = cross_up.y + ly*0.43 ;

	caneva_pts.y[7] = cross_up.y + ly*0.47 ;	//encadre la 4eme ligne d'imagette dessinée main
	caneva_pts.y[8] = cross_up.y + ly*0.56 ;

	caneva_pts.y[9] = cross_up.y + ly*0.6;	//encadre la 5eme ligne d'imagette dessinée main
	caneva_pts.y[10] = cross_up.y + ly*0.68;

	caneva_pts.y[11] = cross_up.y + ly*0.72 ;	//encadre la 6eme ligne d'imagette dessinée main
	caneva_pts.y[12] = cross_up.y + ly*0.81;
