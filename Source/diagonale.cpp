...
Diagonale::Diagonale(string nameCaract){
	Caracteristics();
	name = nameCaract;
}

void Diagonale::setCaractValues(map<string,vector<string>> pathFiles){
	cout << "diagonale" << endl;
	...
	for(map<string,vector<string>>::iterator it=pathFiles.begin(); it!=pathFiles.end(); ++it){
		...
		for(int i=0;i<pathsForAPicto.size();i++){	
			...
			temp.push_back(calcul_diagonale(src));
		}
...
Mat Diagonale::normalize_img(Mat src){
...
double Diagonale::calcul_diagonale(Mat src){
	...
	result = 100*result/(hauteur+longueur);
	return result;
}